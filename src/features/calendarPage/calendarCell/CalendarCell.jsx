...
export const CalendarCell = memo(
  ({ dayCell, setViewedCell, setViewModeAddModal, setViewModeEditModal }) => {
    ...
    return (
      <div
        onDoubleClick={() => (!canClick ? onDoubleClick() : () => false)}
        className={`calendar__table__cell ${
          isCurrentDay(dayCell) ? "calendar__table__cell_today" : null
        }`}
      >
        <div
          onClick={() => (canClick ? handleClick() : () => false)}
          className={`calendar__table__cell__day  ${
            note.length ? "calendar__table__cell__day_withNote" : ""
          }`}
        >
          <div className="calendar__table__cell__day__data">
            {dayCell.dayName ? (
              <span className="calendar__table__cell__day__data__name">
                {dayCell.dayName}
              </span>
            ) : (
              <span> </span>
            ...
            <span
              className={`calendar__table__cell__day__data__number ${
                isDaysFromSelectedMonth(dayCell)
                  ? "calendar__table__cell__day__number_current"
                  : ""
              }`}
            >
              {dayCell.format("D")}
